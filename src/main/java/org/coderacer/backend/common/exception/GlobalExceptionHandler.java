...
public class GlobalExceptionHandler {
  ...
  public ResponseEntity<ProblemDetails> handleValidation(ValidationException ex, HttpServletRequest request) {
    return buildResponse(HttpStatus.BAD_REQUEST, "Validation failed", ex.getCode(), request, ex.getErrors());
  }
  ...
  public ResponseEntity<ProblemDetails> handleMethodArgumentNotValid(MethodArgumentNotValidException ex, HttpServletRequest request) {
    List<FieldError> errors = ex.getBindingResult().getFieldErrors().stream()
        ...
        .toList();
    return buildResponse(HttpStatus.BAD_REQUEST, "Validation failed", "INVALID_INPUT", request, errors);
  }
  ...
  public ResponseEntity<ProblemDetails> handleBaseException(BaseException ex, HttpServletRequest request) {
    log.error("Domain exception occurred: {}", ex.getMessage());
    return buildResponse(HttpStatus.INTERNAL_SERVER_ERROR, ex.getMessage(), ex.getCode(), request, null);
  }
